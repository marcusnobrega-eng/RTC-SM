...
function [F_d,h_ef_w,inflows,ETR,k_out,v] = wshed_matrix(d_0,h_0,inflows,time_step,Direction_Matrix,i_0,ksat,psi,dtheta,I_0,Lambda,Delta_x,Delta_y,ETP,idx_fdir,k_out,pos)
% Faster way
...
if max(inflows) <= 1e-2 % mm/hr
    ...
    h_ef_w = round(d_0 + time_step/3600*(i_0 - ETP/24 - inf_rate),8);
...
%     inflow_rate = i_0  + delta_q +  d_0/(time_step/3600); % mm/h with NO ETP
    ...
    % ---- Effective Depth of Surface Runoff --- %
    %     h_ef_w = round(d_0 + time_step/3600*(i_0 - ETP/24 - inf_rate),5);
%     h_ef_w_half = round(d_0 + time_step/3600*(i_0 + delta_q - ETP/24 - inf_rate),8);
%     [inflows,~,~] = non_lin_reservoir(Lambda,(h_ef_w),h_0,Delta_x,Delta_y); % flow_t = flow(k+1) (outflow from each cell)
    
    delta_q = (Direction_Matrix)*(inflows).*idx_fdir; % This can be time expensive
    h_ef_w = round(d_0 + time_step/3600*(i_0 + delta_q - ETP/24 - inf_rate),8);
    ...
    if min(F_d) < F_d_min_value
        ...
        soil_flux(idx1) = (F_d_min(idx1) - I_0(idx1))/(time_step/60);
        ETR(idx1) = round((-1)*(soil_flux(idx1) - inf_rate(idx1))*24,8); % Everything becomes ETP
        k_out(idx1) = 0; % No grundwater replenishing
