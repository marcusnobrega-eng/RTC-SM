...
% time-step
function [max_water_level,h_c] = channel_dynamics(out_r,time_step,h_c_0,x_length,y_length,roughness,average,variance,segments,s,slope_out_channel,h_end)
...
h_c = zeros(n_cells,steps_horizon); h_c(:,1) = h_0_c; 
...
wshed_inflow = zeros(n_cells,steps_horizon); wshed_inflow(1,1) = out_r(1);
inflow = outflow(h_c(:,1),x_cells,roughness,slope);
    for k = 1:(steps_horizon-1)
        ...
        if min(min(slope)) < 0
            error('Negative slopes meaning flow would change direction. This is not programmed in this model.')
        end
        ...
        wshed_inflow(1,k+1) = out_r(k+1); % Assuming the inflow from the cathcment only for the 1st cell
